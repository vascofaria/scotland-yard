...
class SearchProblem:
  ...
  def search(self, init, limitexp = 2000, limitdepth = 10, tickets = [math.inf,math.inf,math.inf], anyorder = False):
    ...
    if (len(init) == 1):
      ...
      goalsPermutations = [[self.goal[0], self.goal[1], self.goal[2]], [self.goal[0], self.goal[2], self.goal[1]], [self.goal[1], self.goal[0], self.goal[2]], [self.goal[1], self.goal[2], self.goal[0]], [self.goal[2], self.goal[0], self.goal[1]], [self.goal[2], self.goal[1], self.goal[0]]]
      min_path = math.inf
      selectedPath = []
  ...
  def bfs3(self, init, limitexp, limitdepth):
    ...
    while not done and numexp < limitexp and depth < limitdepth:
  ...
  def bfs1(self, srcVertex, limitexp, limitdepth):
    ...
    while len(queue) != 0 and not done and numexp < limitexp:
      current = queue.pop(0)
      numexp += 1
